...
/**
 * Market Pulse placeholder section.
...
export default function MarketPulse() {
  ...
  const signalCount = data.demandSignals.length;
  const costCount = data.inputCosts.length;
  const rawSignalCount = rawData.demandSignals.length;
  const rawCostCount = rawData.inputCosts.length;
    ...
    <div className="p-md space-y-md">
      ...
      <div className="bg-surface-raised border border-surface-overlay rounded p-md space-y-sm">
        <div className="text-xs text-text-secondary">
          ...
          {" | "}
          <span className="font-medium text-text-primary">
            {costCount} input costs
          </span>
          {(signalCount !== rawSignalCount || costCount !== rawCostCount) && (
            <span className="text-text-muted">
              {" "}
              (of {rawSignalCount} signals, {rawCostCount} costs total)
            </span>
          )}
        </div>

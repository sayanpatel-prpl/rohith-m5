...
// ---------------------------------------------------------------------------
// safeDisplay (DATA-04: Graceful Degradation)
// ---------------------------------------------------------------------------

describe("safeDisplay", () => {
  it("formats valid numeric values", () => {
    const result = safeDisplay(12.5, formatPercent);
  ...
  it("returns dash for null values", () => {
    const result = safeDisplay(null, formatPercent);
    expect(result.text).toBe("-");
    expect(result.hasData).toBe(false);
    expect(result.tooltip).toBe("Data not available");
  });

  it("returns dash for undefined values", () => {
    const result = safeDisplay(undefined, formatPercent);
    expect(result.text).toBe("-");
    expect(result.hasData).toBe(false);
  });

  it("returns dash for NaN values", () => {
    const result = safeDisplay(NaN, formatPercent);
    expect(result.text).toBe("-");
    expect(result.hasData).toBe(false);
  });

  it("returns dash for Infinity values", () => {
    const result = safeDisplay(Infinity, formatPercent);
    expect(result.text).toBe("-");
    expect(result.hasData).toBe(false);
  });

  it("uses custom tooltip when provided", () => {
    const result = safeDisplay(null, formatPercent, "Revenue data pending");
    expect(result.tooltip).toBe("Revenue data pending");
  });

  it("handles zero as valid data", () => {
    const result = safeDisplay(0, formatPercent);
    expect(result.text).toBe("0.0%");
    expect(result.hasData).toBe(true);
  });

  it("works with formatINRCr formatter", () => {
    const result = safeDisplay(1500, formatINRCr);
    expect(result.text).toBe("INR 1,500 Cr");
    expect(result.hasData).toBe(true);
  });
});

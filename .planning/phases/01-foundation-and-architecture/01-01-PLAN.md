...
files_modified:
  ...
  - public/brands/pricio/logo.svg
  - public/brands/pricio/favicon.ico
...
must_haves:
  truths:
    - "Navigating to /pricio/report shows Pricio branding (logo, accent color, font)"
    - "Navigating to /bcg/report shows BCG branding (different logo, BCG green accent, Georgia font)"
    - "Switching from /pricio/report to /bcg/report changes all branding without page reload"
    ...
    - "Unknown tenant slug (e.g., /unknown/report) falls back to Pricio branding"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    4. **Create `src/theme/tokens.css`** — This is the CRITICAL file. Follow the RESEARCH.md Pattern 1 exactly:
       ...
       - `@layer base` with:
         ...
         - `[data-tenant="pricio"]` overriding `--color-brand-accent` (Pricio blue-purple)
    ...
    6. **Update `index.html`** — Add blocking dark mode script in `<head>` BEFORE React loads (prevents white flash — see RESEARCH.md Pitfall 4):
       ...
       ```
       Also set `<link rel="icon" href="/brands/pricio/favicon.ico" />` as default.
    ...
    7. **Create brand config files:**
       ...
       - `src/brands/pricio.ts`: Pricio Intelligence config (default/demo brand, blue-purple accent, Inter font)
       ...
       - `src/brands/index.ts`: Brand registry `Record<string, BrandConfig>` + `getBrandConfig(slug)` function that falls back to Pricio for unknown slugs

    8. **Create placeholder brand assets:**
       - `public/brands/pricio/logo.svg`: Simple SVG text logo saying "Pricio Intelligence" in a professional style
       - `public/brands/pricio/favicon.ico`: Generate a simple 16x16 favicon (can be a colored square)
  ...
  <done>
    Vite dev server runs. TypeScript compiles with zero errors. tokens.css generates all design token CSS custom properties. Three brand configs exist with fallback to Pricio. Dark mode blocking script is in index.html head. `initTheme()` is called in main.tsx.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    1. **Create `src/components/brand/BrandProvider.tsx`** — Follow RESEARCH.md Pattern 1:
       ...
       - Calls `getBrandConfig(tenantSlug ?? "pricio")` to resolve brand
    ...
    4. **Create `src/app/App.tsx`** — React Router setup (declarative mode, NOT framework mode):
       ...
       - Route `/` redirects to `/pricio/report`
       - Catch-all `*` redirects to `/pricio/report`
    ...
    6. **Create `src/components/layout/TopBar.tsx`** — Slim header bar:
       ...
       - Center/Right: Edition badge placeholder ("February 2026 Edition") — just static text for now, styled with brand accent
       - Right: Dark mode toggle button (cycles: light -> dark -> system, calls `applyTheme` from dark-mode.ts)
  ...
  <verify>
    ...
    1. Navigate to `http://localhost:5173/pricio/report` — see Pricio logo and blue-purple accent
    ...
    4. Navigate to `http://localhost:5173/` — redirects to `/pricio/report`
    ...
    7. Toggle dark mode — all surfaces and text switch themes, preference persists after page refresh
    8. Run `npx tsc --noEmit` — zero TypeScript errors
  </verify>
  <done>
    App shell renders with TopBar (tenant logo + edition badge + dark mode toggle) and Sidebar (10 sections with active state). Navigating between /pricio/report, /bcg/report, and /am/report switches all branding (logo, accent colors, typography) without page reload. Dark mode toggle works and persists. Each section is wrapped in an error boundary. Unknown tenant slugs fall back to Pricio. The app compiles and runs with zero TypeScript errors.
  </done>
</task>
...
3. Navigate to /pricio/report — Pricio branding visible (logo, colors)
...
7. Dark mode toggle cycles light/dark/system, preference persists on refresh
8. No dark mode flash on page load (blocking script works)
9. Navigate to /unknown-slug/report — falls back to Pricio branding
10. Navigate to / — redirects to /pricio/report
</verification>

<success_criteria>
- Multi-tenant BrandProvider resolves tenant from URL slug and injects brand tokens as CSS custom properties
- Three brand configs (pricio, bcg, am) with distinct accent colors and fonts
- App shell with slim top bar, left sidebar (10 sections), and routed content area
- Dark mode support with three-way toggle and localStorage persistence

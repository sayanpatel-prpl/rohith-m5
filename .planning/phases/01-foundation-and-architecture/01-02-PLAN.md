...
<task type="auto">
  ...
  <action>
    ...
    11. **Update TopBar to include EditionBadge:** Import and render `<EditionBadge edition="February 2026" />` in the TopBar component from Plan 01.
  </action>
  <verify>
    Run `npx tsc --noEmit` — all components compile cleanly.
    Run `npm run dev` — verify in browser:
    1. EditionBadge appears in TopBar
...
9. Switching tenant URL changes chart accent/annotation colors
10. EditionBadge shows edition text in TopBar
11. DataRecencyTag renders data freshness indicator
12. ChartTooltip displays formatted values on hover
</verification>
...
- Chart annotations and tooltips work for inline data callouts

...
must_haves:
  truths:
    ...
    - "src/api/news.ts re-exports news functions from data/loaders/news.ts (NEWS-07)"
  artifacts:
    ...
    - path: "dashboard_build_v2/src/api/news.ts"
      provides: "Public API surface for news data (NEWS-07)"
      exports: ["loadNewsItems", "getNewsForSection", "getNewsForCompany", "isCorroborated", "isContradicted"]
  key_links:
    ...
    - from: "dashboard_build_v2/src/api/news.ts"
      to: "dashboard_build_v2/src/data/loaders/news.ts"
      via: "re-export"
      pattern: "export.*from.*data/loaders/news"
---
...
<task type="auto">
  <name>Task 1: Create data loaders, company registry, and news API module</name>
  <files>
    dashboard_build_v2/src/data/loaders/consolidated.ts
    dashboard_build_v2/src/data/loaders/screener.ts
    dashboard_build_v2/src/data/loaders/sovrenn.ts
  ...
  <action>
    ...
    **src/api/news.ts (NEWS-07):** Create the public API surface for news data. This file re-exports all functions from `../data/loaders/news.ts` so that consumers can import from `src/api/news.ts` as the requirement specifies:
    ```typescript
    export {
      loadNewsItems,
      getNewsForSection,
    ...
    ```
    This ensures NEWS-07's requirement for src/api/news.ts is met while keeping the loader implementation in the data layer.
  </action>
  <verify>
    ...
    Verify src/api/news.ts re-exports all functions from data/loaders/news.ts.
  </verify>
  <done>
    All 6 data loaders import their respective JSON files and export typed accessor functions. Company registry covers all 16 companies with lookup by ID, ticker, or name. News loader has anti-clickbait filtering and graceful degradation for missing data. src/api/news.ts provides the public API surface per NEWS-07.
  </done>
</task>
...
8. src/api/news.ts re-exports from data/loaders/news.ts per NEWS-07
9. `npx tsc --noEmit` passes for all files in this plan
</verification>
...
- src/api/news.ts exists as public API surface re-exporting from data/loaders/news.ts

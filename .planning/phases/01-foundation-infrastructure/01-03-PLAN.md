...
must_haves:
  truths:
    ...
    - "CorroboratedBadge renders a visual indicator for corroborated news items (NEWS-03)"
    - "ConflictingReportsTag renders a visual indicator for contradicted news items (NEWS-04)"
  ...
  artifacts:
    ...
    - path: "dashboard_build_v2/src/components/source/TierBadge.tsx"
      ...
      exports: ["TierBadge"]
    - path: "dashboard_build_v2/src/components/source/CorroboratedBadge.tsx"
      provides: "Badge showing news item is corroborated by multiple sources (NEWS-03)"
      exports: ["CorroboratedBadge"]
    - path: "dashboard_build_v2/src/components/source/ConflictingReportsTag.tsx"
      provides: "Tag indicating contradicted/conflicting reports (NEWS-04)"
      exports: ["ConflictingReportsTag"]
  ...
  key_links:
    ...
    - from: "dashboard_build_v2/src/components/source/CorroboratedBadge.tsx"
      to: "dashboard_build_v2/src/types/news.ts"
      via: "NewsItem type import"
...
<objective>
Build all shared UI components: source attribution system (including corroborated/conflicting indicators), A&M service line tags, chart and table wrappers, and common UI primitives.

Purpose: These are cross-cutting components used by every section. Source attribution (SRCA-01 through SRCA-04) is required on every card, chart, and table. News credibility indicators (NEWS-03, NEWS-04) provide visual cues for corroborated and contradicted signals. ECharts and TanStack Table wrappers ensure consistent chart/table patterns. DataValue ensures graceful degradation.
...
<task type="auto">
  <name>Task 1: Create source attribution, news credibility indicators, and A&M service line components</name>
  <files>
    dashboard_build_v2/src/components/source/SourceAttribution.tsx
    dashboard_build_v2/src/components/source/TierBadge.tsx
    dashboard_build_v2/src/components/source/AMServiceLineTag.tsx
  ...
  <action>
    ...
    **SourceAttribution.tsx (SRCA-01, SRCA-03):** Create the reusable source attribution component.
    ...
    - Confidence badges: "verified" gets a checkmark icon, "derived" gets a calculation icon, "estimated" gets a tilde icon. Use unicode: verified=check, derived=approx, estimated=tilde.
    ...
    **CorroboratedBadge.tsx (NEWS-03):** Create a badge component for news items corroborated by multiple sources.
    Props: `{ sources: string[]; className?: string }`.
    - Renders a small green-tinted badge with a checkmark icon and text "Corroborated" or "Corroborated (N sources)" if sources.length > 1.
    - Tailwind: `inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400`
    - Tooltip (title attr) shows the corroborating source names joined by ", ".
    - Used alongside news items where isCorroborated() returns true.

    **ConflictingReportsTag.tsx (NEWS-04):** Create a tag component for news items contradicted by other sources.
    Props: `{ sources: string[]; className?: string }`.
    - Renders a small amber/orange-tinted tag with a warning icon and text "Conflicting Reports".
    - Tailwind: `inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400`
    - Tooltip (title attr) shows "Contradicted by: " + source names.
    - Used alongside news items where isContradicted() returns true.

    **index.ts:** Barrel export all five components: SourceAttribution, TierBadge, AMServiceLineTag, CorroboratedBadge, ConflictingReportsTag.
  </action>
  <verify>
    ...
    Verify CorroboratedBadge accepts sources: string[] prop.
    Verify ConflictingReportsTag accepts sources: string[] prop.
  </verify>
  <done>
    SourceAttribution renders with TierBadge, confidence indicator, and source name — ready to be placed on every card/chart/table (SRCA-03). TierBadge has 4-tier styling per SRCA-02. AMServiceLineTag renders A&M service line labels per AMTH-02. CorroboratedBadge shows corroborated signals styling per NEWS-03. ConflictingReportsTag shows contradicted signals per NEWS-04.
  </done>
</task>
...
4. CorroboratedBadge renders green badge with corroborating source names (NEWS-03)
5. ConflictingReportsTag renders amber tag with contradicting source names (NEWS-04)
6. echarts-core.ts imports ONLY from echarts/core, echarts/charts, echarts/components, echarts/renderers
7. DataTable renders sortable headers with sort indicators
8. DataValue shows "-" with tooltip for null/undefined values
9. SectionErrorFallback provides reset/retry capability
10. `npx tsc --noEmit` passes for all files
</verification>

<success_criteria>
- Source attribution system complete: SourceAttribution + TierBadge + AMServiceLineTag + CorroboratedBadge + ConflictingReportsTag
- TierBadge styling matches spec: T1 green, T2 blue, T3 amber outline, T4 red outline + warning
- CorroboratedBadge visually indicates corroborated news signals (NEWS-03)
- ConflictingReportsTag visually indicates contradicted news items (NEWS-04)

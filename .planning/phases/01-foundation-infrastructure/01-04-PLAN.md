...
files_modified:
  ...
  - dashboard_build_v2/src/components/sections/index.ts
...
must_haves:
  truths:
    - "Dashboard builds to a single HTML file with npm run build"
    - "App shell renders sidebar with 11 section nav items in correct order"
    - "Dark mode toggle works without flash of unstyled content"
    - "Filter bar persists filter state to URL search params"
    - "Placeholder section stubs render with useFilteredData hook wired"
...
<objective>
Wire everything together: app shell, routing, branding, section stubs, and tracking files. Verify the complete build produces a working single-file HTML.

Purpose: This plan integrates all Plan 01-03 outputs into a running dashboard. After this plan, the dev server shows a working shell with sidebar navigation, filter bar, and placeholder sections that load real data via the data layer.
Output: Working dashboard shell that builds to single HTML + all tracking files populated.
</objective>
...
<task type="auto">
  <name>Task 1: Create app shell, routing, branding, and section stubs</name>
  <files>
    dashboard_build_v2/src/components/brand/BrandProvider.tsx
    dashboard_build_v2/src/components/brand/useBrand.ts
    dashboard_build_v2/src/components/layout/AppShell.tsx
  ...
  <action>
    ...
    **sections/index.ts (lazySections registry):** Create lazy-loaded section registry. For each section in SECTION_ROUTES, create a React.lazy() import pointing to the section's index.tsx. For now, all 11 sections will be stubs — only executive, financial, and watchlist get slightly more populated stubs (they're Phase 2 priority). The remaining 8 sections get minimal placeholder stubs.

    **Section stubs (executive/index.tsx, financial/index.tsx, watchlist/index.tsx):** Each stub:
    - Default export function component
    - Calls `useFilteredData<SectionData>('sectionId')` to verify the data pipeline works
    - Renders section title, a "Coming in Phase 2" message, and a SourceAttribution component with sample source data
    - Includes `// NEWS_DATA_SLOT` comment (NEWS-05)
    - Includes graceful empty state for news: `const newsItems: NewsItem[] = []; if (newsItems.length === 0) { /* NEWS-06: no empty state rendered */ }` pattern

    For the remaining 8 sections (am-value-add, market-pulse, deals, operations, leadership, competitive, deep-dive, what-this-means), create minimal stubs in their respective directories that just export a default component rendering the section name. Each includes `// NEWS_DATA_SLOT` comment.
    ...
    **App.tsx:** Root component. Port from v1's App.tsx:
    ...
    - Each section route: lazy-loaded via lazySections, wrapped in Suspense with SectionSkeleton fallback, wrapped in ErrorBoundary with SectionErrorFallback
  ...
  <verify>
    Run `cd dashboard_build_v2 && npm run dev` — dev server starts without errors.
    Navigate to http://localhost:5173/am/report — AppShell renders with sidebar showing 11 sections.
    Click "Financial Performance" in sidebar — navigates and shows stub content.
    Toggle dark mode — background and text colors change without FOUC.
    Run `cd dashboard_build_v2 && npm run build` — produces dist/index.html as single file (FOUND-02).
    Verify dist/ contains only index.html (no other files).
    Check dist/index.html file size is reasonable (< 5MB).
  </verify>
  <done>
    Dashboard shell renders with 11-section sidebar (A&M Value-Add at position 2), filter bar, dark mode toggle, and lazy-loaded section stubs. Single-file HTML build produces one index.html. Multi-tenant branding applies via data-tenant attribute.
  </done>
</task>

<task type="auto">
  <name>Task 2: Create tracking files and data catalog</name>
  <files>
    dashboard_build_v2/PROGRESS.md
    dashboard_build_v2/DECISIONS.md
    dashboard_build_v2/DATA_CATALOG.md
...
<verification>
1. `npm run dev` starts and renders the dashboard shell at /am/report
...
6. `npm run build` produces a single index.html in dist/
7. dist/index.html size < 5MB
8. All 4 tracking files exist in dashboard_build_v2/
9. Section stubs load and show useFilteredData working
10. NEWS_DATA_SLOT comments present in section stubs
</verification>

<success_criteria>
- Dashboard shell renders with sidebar, filter bar, dark mode, and 11 section routes
- A&M Value-Add is at nav position 2
- Default tenant is "am", multi-tenant branding works via data-tenant attribute
- Single-file HTML build produces one dist/index.html
- All 4 tracking files (PROGRESS.md, DECISIONS.md, DATA_CATALOG.md, SOURCE_REFERENCE.md) exist and are populated
- Section stubs demonstrate the data pipeline (useFilteredData -> query -> loader -> JSON)
- NEWS_DATA_SLOT comments in all section stubs (NEWS-05)
